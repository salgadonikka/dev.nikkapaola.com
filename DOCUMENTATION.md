...
├── components/
│   ├── SiteHeader.astro     # Top nav bar (logo + nav links) — used by all pages
│   ├── portfolio/           # Portfolio page tile components (Tile01–03, sidebar [unused])
...
├── layouts/
│   ├── SiteLayout.astro     # Shell with top header (SiteHeader)
│   └── BlogPost.astro       # Individual blog post wrapper
├── pages/               # File-based routing (each file = a route)
├── styles/
│   ├── global.css       # Design tokens + base styles
│   ├── portfolio.css    # /portfolio page styles
│   ├── projects.css     # /projects + /about styles
│   ├── landing.css      # / landing page + SiteHeader styles
│   └── site-layout.css  # SiteLayout shell styles
...
The landing page renders its own `<html>` shell with `<BaseHead>` and `<SiteHeader>` directly — it does **not** use `SiteLayout`.

Sections (top to bottom):
- **Hero** — name, role, bio, availability badge, and CTA buttons (Portfolio / Get in Touch)
- **Hero photo** — displays `portfolio-photo.jpg` by default; swaps to `secondary-photo.jpg` on hover via CSS (`.lp-photo-primary` / `.lp-photo-hover`)

Data sourced from `src/data/portfolioData.js` (identity, status, contactLinks).
...
Two-column layout with a prose section on the left and a sticky sidebar on the right.

**Left (prose):**
- **Background** — narrative career overview
- **Career highlights** — EastWest Bank → Deltek → WTW timeline with a link to `/portfolio`
- **Languages** — list from `bio.languages` in `portfolioData.js`

**Right (sidebar cards):**
- **Core stack** — top skills filtered from `skillGroups` (`bp` variant, first 10)
- **Industries** — from `bio.industries` in `portfolioData.js`
- **Say hello** — email and LinkedIn links sourced from `contactLinks`

Data sourced from `src/data/portfolioData.js` (`bio`, `skillGroups`, `contactLinks`).

**Styles:** Scoped `<style>` block within the file; uses `proj-page-header` styles from `projects.css`.
...
Top navigation bar used by all pages. Renders the `NPS.` monogram logo (linking to `/`) and nav links from `visibleNav`. Active route is highlighted via `is-active` class. Styled via `.lp-header` and `.lp-header-nav` classes in `landing.css`.

> This component replaced the original icon-based side rail. `PortfolioSidebar` and `PortfolioMobileBar` still exist in the codebase but are no longer wired into `SiteLayout`.

---

#### `portfolio/PortfolioSidebar.astro`

Desktop icon sidebar navigation. **Currently unused** — replaced by `SiteHeader`. Kept in the codebase but not referenced by any layout.

---

#### `portfolio/PortfolioMobileBar.astro`

Mobile bottom navigation bar. **Currently unused** — replaced by `SiteHeader`. Kept in the codebase but not referenced by any layout.
...
Shell layout used by all non-blog pages (`/portfolio`, `/projects`, `/about`, `/search`). Note: the landing page (`/`) uses `SiteHeader` directly rather than this layout.
...
- Main content slot (`sl-content` div)

**Styles:** `src/styles/site-layout.css`, `src/styles/landing.css`

> **Note:** The original icon-based side rail (`PortfolioSidebar` / `PortfolioMobileBar`) has been replaced by the `SiteHeader` top bar. The rail code remains in the file but is commented out.
